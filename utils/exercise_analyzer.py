...
class ExerciseAnalyzer:
    ...
    def _is_visible(self, kp, idx):
        """Check if keypoint is visible. YOLO uses 0,0 for missing points"""
        ...
        for side in ("right", "left"):
            ...
            if zone == "down" and s["stable"] >= self.STABILITY_FRAMES:
                if s["curl_state"] == "up":
                    ...
                    rep_done = True
                s["curl_state"] = "down"
                feedback = f"{int(angle)}deg - Now curl UP!"
            elif zone == "up" and s["stable"] >= self.STABILITY_FRAMES:
                s["curl_state"] = "up"
                feedback = f"{int(angle)}deg - Lower back DOWN!"
            else:
                feedback = f"{int(angle)}deg - Moving..."
        ...
        for side in ("right", "left"):
            ...
            if zone == "up" and s["stable"] >= self.STABILITY_FRAMES:
                if s["squat_state"] == "down":
                    ...
                    rep_done = True
                s["squat_state"] = "up"
                feedback = f"{int(angle)}deg - Squat DOWN!"
            elif zone == "down" and s["stable"] >= self.STABILITY_FRAMES:
                s["squat_state"] = "down"
                feedback = f"{int(angle)}deg - Stand UP!"
            else:
                feedback = f"{int(angle)}deg - Moving..."
