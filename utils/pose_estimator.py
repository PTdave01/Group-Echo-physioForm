...
class PoseEstimator:
    ...
    def get_keypoints(self, img):
        """Return keypoints (17,2) and bounding box for the main person."""
        self.frame_count += 1

        # Frame skipping – reuses last detection for speed
        if self.frame_count % self.skip_frames != 0:
            return self.last_keypoints, self.last_bbox

        h, w = img.shape[:2]

        # Resize image for faster inference (192px on the longest side)
        scale = 192 / max(h, w)
        small = cv2.resize(img, (int(w * scale), int(h * scale)),
                           interpolation=cv2.INTER_LINEAR)

        # Run YOLOv8n-pose (verbose=False to avoid logs)
        results = self.model(small, imgsz=192, verbose=False, conf=0.25)

        if not results or len(results[0].keypoints) == 0:
            return self.last_keypoints, self.last_bbox

        # Extract keypoints of the first (most confident) person
        kpts_raw = results[0].keypoints.xy[0].cpu().numpy()
    ...
    def draw_skeleton(self, img, keypoints, line_colors=None):
        """Draw skeleton lines on the image."""
