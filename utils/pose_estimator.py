...
def get_model():
    global _model
    if _model is None:
        _model = YOLO("yolov8n-pose.pt") # loads once
    return _model

class PoseEstimator:
    def __init__(self):
        self.model = get_model()
        self.skeleton = [(5,6),(5,7),(7,9),(6,8),(8,10),(5,11),(6,12),(11,12),(11,13),(13,15),(12,14),(14,16)]

    def get_keypoints(self, img):
        h, w = img.shape[:2]
        # --- SPEED HACK: run pose on 320px max ---
        scale = 320 / max(h, w)
        small = cv2.resize(img, (int(w*scale), int(h*scale)), interpolation=cv2.INTER_LINEAR)

        results = self.model(small, imgsz=320, verbose=False, conf=0.35, iou=0.5)
        if not results or len(results[0].keypoints) == 0:
            return None, None

        kpts = results[0].keypoints.xy[0].cpu().numpy() / scale # scale back up
    ...
    def draw_skeleton(self, img, keypoints, line_colors=None):
        if keypoints is None:
            return img
        # draw directly - no overlay copy (saves 30% CPU)
        for i,j in self.skeleton:
            if i>=len(keypoints) or j>=len(keypoints): continue
            x1,y1 = keypoints[i].astype(int); x2,y2 = keypoints[j].astype(int)
